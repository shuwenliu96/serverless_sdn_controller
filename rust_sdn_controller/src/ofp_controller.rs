...
pub trait OfpController {
    ...
    fn handle_client_connected(_: &mut TcpStream, _: HashMap<u32,u32>);
}
...
pub mod openflow0x01 {
        ...
        fn process_message(&mut self,
            ...
            match msg {
                ...
                Message::EchoRequest(bytes) => {
                    ...
                    //Cntl::send_message(xid, Message::EchoRequest(vec![0]), stream);
                }
                Message::EchoReply(_) => println!("EchoReply"),
                Message::FeaturesReq => (),
                Message::FeaturesReply(feats) => {
                    if self.switch_id.is_some() {
                        panic!("Switch connection already received.")
                    }
                    //println!("Features: {}, {}, {}", feats.datapath_id, feats.num_buffers, feats.num_tables);
                    self.switch_id = Some(feats.datapath_id);
                    //Cntl::switch_connected(cntl, feats.datapath_id, feats, stream, remote)
                }
                Message::FlowMod(_) => (),
    ...
    pub trait OF0x01Controller: OfpController<Message = Message> {
        ...
        fn new(policy: HashMap<u32, u32>) -> Self;
        /// Callback invoked with `sw` when a switch with identifier `sw` connects to
        /// the controller.
        fn switch_connected(&mut self, sw: u64, feats: SwitchFeatures, stream: &mut TcpStream, remote: &mut TcpStream);
    ...
    impl<Controller: OF0x01Controller> OfpController for Controller {
        ...
        fn handle_client_connected(stream: &mut TcpStream, policy: HashMap<u32, u32>) {
            let mut cntl = Controller::new(policy);
            ...
            loop {
                //Controller::send_message(0, Message::EchoRequest(vec![0]), stream);
                ...
                match res {
                    Ok(num_bytes) if num_bytes > 0 => {
                        ...
                        if message_len != 0 {let _ = stream.read(&mut message_buf);}
                        
                        //println!("len: {}, header size: {}, message buf {:?}", header.length(), OfpHeader::size(), message_buf);
